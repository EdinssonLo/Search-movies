...
export const fetchMovieRatings = (id) => async (dispatch) => {
 try {
  ...
  const response = await fetch(`${BASE_URL}/get-ratings?tconst=${id}`, { headers });
...
export const fetchMovieDetail = (id) => async (dispatch) => {
 try {
  ...
  const overviewDetailsResponse = await fetch(`${BASE_URL}/get-overview-details?tconst=${id}`, { headers });
  const overviewDetailsData = await overviewDetailsResponse.json();
  console.log(overviewDetailsData)

  const topCastResponse = await fetch(`${BASE_URL}/get-top-cast?tconst=${id}`, { headers });
  const topCastData = await topCastResponse.json();

  const detailsResponse = await fetch(`${BASE_URL}/get-details?tconst=${id}`, { headers });
  const detailsData = await detailsResponse.json();

  const fullCreditsResponse = await fetch(`${BASE_URL}/get-full-credits?tconst=${id}`, { headers });
